...
class BaseWKWebViewController: UIViewController {
    ...
    var frame = CGRect.init(x: 0, y: 64, width: ScreenWidth, height: ScreenHeight - 64)
...
// MARK: - 外部方法使用
extension BaseWKWebViewController{
    /// 普通URL加载方式
    ///
...
extension BaseWKWebViewController{
    ...
    fileprivate func addProgressView() {
        ...
        progressView?.progressTintColor = UIColor.init(red: 21/255.0, green: 126/255.0, blue: 251/255.0, alpha: 1.0)
...
extension BaseWKWebViewController: WKScriptMessageHandler{
        ...
        if mobileUrl != nil {
            if mobileUrl!.hasPrefix("tel") {
                ...
                if #available(iOS 10.0, *) {
                    ...
                    //高于 iOS 10.0
                    UIApplication.shared.open(URL.init(string: mobileUrl!)!, options: [:], completionHandler: { (success) in
